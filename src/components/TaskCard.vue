...
const progressPercentage = computed(() => {
  if (!props.task.quantity || !props.task.progress || props.task.quantity === 0)
    return 0;
    ...
    <div class="flex items-start gap-4">
      ...
      <div class="flex-1">
        ...
        <div class="flex flex-wrap items-center gap-2 mt-3 text-xs">
          ...
          <!-- Goal Badge -->
          <span
            v-if="(task as any).goalName"
            class="px-2 py-0.5 rounded-full bg-orange-50 text-orange-700 dark:bg-badge-bg dark:text-badge-text font-medium"
          >
        ...
        <!-- Progress Bar -->
        <div
          v-if="task.progress !== undefined && task.quantity"
        ...
        >
          <div class="flex justify-between items-center text-xs">
            <span class="text-neutral-500 flex items-center">
              Progress:
              <span
                class="inline-flex items-center gap-2 ml-2 bg-neutral-100 dark:bg-neutral-800 rounded-md px-1 py-0.5"
              >
                <button
                  @click="handleUpdateProgress(false)"
                  class="p-0.5 text-neutral-400 hover:text-neutral-900 hover:bg-neutral-200 dark:hover:bg-neutral-700 rounded transition-colors"
                  :disabled="isPending"
                >
                  <Minus :size="14" />
                </button>
                <span
                  class="font-medium text-neutral-700 dark:text-neutral-300 min-w-[3rem] text-center"
                >
                  {{ formatProgress(task.progress) }} /
                  {{ formatProgress(task.quantity) }}
                  <span v-if="task.goalUnitCode !== 't'">{{
                    task.goalUnitName?.toLowerCase()
                  }}</span>
                </span>
                <button
